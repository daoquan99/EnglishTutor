...
public static class ScenarioErrors
{
    public static Error NotFound(Guid scenarioId) =>
        Error.NotFound("Learning.ScenarioNotFound", $"Scenario '{scenarioId}' was not found.");

    public static Error DuplicateName(string name) =>
        Error.Conflict("Learning.ScenarioDuplicateName", $"Scenario with name '{name}' already exists for this Topic + Mode combination.");
