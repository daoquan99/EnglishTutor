...
{
    public static async Task MigrateApiDatabasesAsync(this WebApplication app)
    {
        var autoMigrate = app.Configuration.GetValue("Database:AutoMigrate", app.Environment.IsDevelopment());
        var validateMigrations = app.Configuration.GetValue("Database:ValidateMigrations", true);
        ...
        where TDbContext : DbContext
    {
        ...
        if (pendingMigrations.Length > 0)
        {
            throw new InvalidOperationException(
                ...
                "Apply migrations before starting the API or enable Database:AutoMigrate for local development.");
        }
    }
}
