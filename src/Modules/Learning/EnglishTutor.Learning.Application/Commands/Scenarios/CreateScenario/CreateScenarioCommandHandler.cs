...
public sealed class CreateScenarioCommandHandler : ICommandHandler<CreateScenarioCommand, Guid>
{
    ...
    public async Task<Result<Guid>> Handle(CreateScenarioCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!topic.IsActive)
        {
            return Result.Failure<Guid>(new Error("Learning.TopicInactive", $"Topic '{request.TopicId}' is inactive."));
        }
        ...
        if (!mode.IsActive)
        {
            return Result.Failure<Guid>(new Error("Learning.ModeDefinitionInactive", $"Mode definition '{request.ModeDefinitionId}' is inactive."));
        }
