...
public sealed class EndPracticeSessionHandler : ICommandHandler<EndPracticeSessionCommand, EndSessionResult>
{
    ...
    public EndPracticeSessionHandler(
    ...
    public async Task<Result<EndSessionResult>> Handle(EndPracticeSessionCommand request, CancellationToken ct)
    {
        var session = await _sessions.GetByIdAsync(request.SessionId, ct);
        if (session is null)
        {
            return Result.Success(new EndSessionResult(EndSessionStatus.SessionNotFound, null, null, 0, "practice.end.not_found"));
        }

        if (session.UserId != request.UserId)
        {
            return Result.Success(new EndSessionResult(EndSessionStatus.Forbidden, null, null, 0, "practice.end.forbidden"));
        }
