...
public sealed class UserVocabularyMastery : AggregateRoot<Guid>
{
    ...
    public bool CanMarkMastered => Status != VocabularyMasteryStatus.New
        && Status != VocabularyMasteryStatus.Mastered
        && ReviewCount >= 3
        && ConsecutiveCorrectCount >= 2
        && MeaningMasteryScore >= 85;
    ...
    public void MarkMastered(DateTime utcNow)
    {
        ...
        if (!CanMarkMastered)
        {
            throw new DomainException(
                "Cannot mark as mastered. Requires: score >= 85, 3+ reviews, 2+ consecutive correct, and status not New.");
        }
    ...
    public void RecordExampleAttempt(int score, DateTime utcNow)
    {
        var normalizedScore = Math.Clamp(score, 0, 100);
        ExampleSentenceScore = CalculateMovingScore(ExampleSentenceScore, normalizedScore);

        AddDomainEvent(new ExampleFillBlankAttemptedDomainEvent(
            UserId,
            VocabularyItemId,
            Guid.Empty,
            TargetLanguageCode.Value,
