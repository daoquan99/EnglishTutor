    ...
    : DbContext(options), IVocabularyUnitOfWork
{
    ...
    public DbSet<VocabularyStudySettings> VocabularyStudySettings => Set<VocabularyStudySettings>();
    public DbSet<ExampleFillBlankAttempt> ExampleFillBlankAttempts => Set<ExampleFillBlankAttempt>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<VocabularyStudySettings>(builder =>
        {
            builder.ToTable("VocabularyStudySettings");
            builder.HasKey(s => s.Id);
            builder.Property(s => s.TargetLanguageCode)
                .HasConversion(code => code.Value, value => BuildingBlocks.SharedKernel.LanguageCode.Create(value))
                .HasMaxLength(3);
            builder.HasIndex(s => new { s.UserId, s.TargetLanguageCode }).IsUnique();
        });

        modelBuilder.Entity<ExampleFillBlankAttempt>(builder =>
        {
            builder.ToTable("ExampleFillBlankAttempts");
            builder.HasKey(a => a.Id);
            builder.Property(a => a.TargetLanguageCode)
                .HasConversion(code => code.Value, value => BuildingBlocks.SharedKernel.LanguageCode.Create(value))
                .HasMaxLength(3);
            builder.Property(a => a.UserAnswer).HasMaxLength(500);
            builder.Property(a => a.CorrectAnswer).HasMaxLength(500);
            builder.HasIndex(a => new { a.UserId, a.VocabularyExampleId });
        });
        ...
        foreach (var holder in holders)
        {
            foreach (var domainEvent in holder.DomainEvents)
            {
                IntegrationEvent? integrationEvent = domainEvent switch
                {
                    ...
                    ExampleFillBlankAttemptedDomainEvent attempted => new ExampleFillBlankAttemptedIntegrationEvent(
                        attempted.UserId,
                        attempted.VocabularyItemId,
                        attempted.VocabularyExampleId,
                        attempted.TargetLanguageCode,
                        attempted.IsCorrect,
                        attempted.Score,
                        attempted.AttemptedAtUtc)
                    {
                        EventId = attempted.EventId,
                        OccurredOnUtc = attempted.OccurredOnUtc
                    },
                    _ => null
                };
