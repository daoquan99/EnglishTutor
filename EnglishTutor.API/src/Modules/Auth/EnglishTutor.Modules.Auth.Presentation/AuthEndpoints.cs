...
using EnglishTutor.Modules.Auth.Application.Commands.RestoreUser;
using EnglishTutor.Modules.Auth.Application.Commands.SetUserRoles;
using EnglishTutor.Modules.Auth.Application.Commands.SuspendUser;
...
using EnglishTutor.Modules.Auth.Application.Commands.UpdateUser;
...
using EnglishTutor.Modules.Auth.Application.Queries.GetUserDetail;
...
using EnglishTutor.Modules.Auth.Application.Queries.ListUsers;
...
public static class AuthEndpoints
{
    public static IEndpointRouteBuilder MapAuthEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        adminGroup.MapGet("/users", async (
            ...
            CancellationToken ct) =>
        {
            if (!currentUser.HasPermission(PermissionCodes.AuthUsersRead))
            {
                return Results.Forbid();
            }

            return (await sender.Send(new ListUsersQuery(
                search,
                status ?? UserStatusFilter.All,
                roleId,
                page ?? 1,
                pageSize ?? 20,
                sort ?? UserSortOrder.NewestFirst), ct)).ToHttpResult();
        })
            .WithName("ListAuthUsers");

        adminGroup.MapGet("/users/{userId:guid}", async (
            Guid userId,
            ICurrentUser currentUser,
            ISender sender,
            CancellationToken ct) =>
        {
            ...
            return (await sender.Send(new GetUserDetailQuery(userId), ct)).ToHttpResult();
        })
            .WithName("GetAuthUserDetail");

        adminGroup.MapPut("/users/{userId:guid}", async (
            Guid userId,
            ...
            CancellationToken ct) =>
        {
            if (!currentUser.HasPermission(PermissionCodes.AuthUsersManage))
            {
                return Results.Forbid();
            }

            return (await sender.Send(new UpdateUserCommand(userId, request.DisplayName), ct)).ToHttpResult();
        })
            .WithName("UpdateAuthUser");

        adminGroup.MapPost("/users/{userId:guid}/suspend", async (
            Guid userId,
            ...
            CancellationToken ct) =>
        {
            ...
            return (await sender.Send(new SuspendUserCommand(userId, request?.Reason), ct)).ToHttpResult();
        })
            .WithName("SuspendAuthUser");

        adminGroup.MapPost("/users/{userId:guid}/restore", async (
            Guid userId,
            ICurrentUser currentUser,
            ISender sender,
            CancellationToken ct) =>
        {
            ...
            return (await sender.Send(new RestoreUserCommand(userId), ct)).ToHttpResult();
        })
            .WithName("RestoreAuthUser");

        adminGroup.MapPut("/users/{userId:guid}/roles", async (
            Guid userId,
            ...
            CancellationToken ct) =>
        {
            ...
            return (await sender.Send(new SetUserRolesCommand(userId, request.RoleIds), ct)).ToHttpResult();
        })
            .WithName("SetAuthUserRoles");
