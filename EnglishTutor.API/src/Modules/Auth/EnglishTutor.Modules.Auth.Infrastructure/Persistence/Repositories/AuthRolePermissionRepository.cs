...
public sealed class AuthRolePermissionRepository(AuthDbContext dbContext) : IAuthRolePermissionRepository
{
    ...
    public async Task<bool> AreRoleIdsValidAsync(IReadOnlyCollection<Guid> roleIds, CancellationToken cancellationToken)
    {
        if (roleIds.Count == 0)
        {
            return true;
        ...
        var existingCount = await dbContext.AuthRoles
            .CountAsync(
                role => distinctRoleIds.Contains(role.Id) && role.IsEnabled,
                cancellationToken);

        return existingCount == distinctRoleIds.Length;
    }
    ...
    public async Task<IReadOnlyCollection<Guid>> GetUserIdsAssignedToRoleAsync(Guid roleId, CancellationToken cancellationToken) =>
        await dbContext.AuthUserRoles
            .Where(userRole => userRole.RoleId == roleId)
            .Select(userRole => userRole.AuthUserId)
            .Distinct()
            .ToListAsync(cancellationToken);
