...
public sealed class VocabularyMasteredEventHandler(
    ...
    : IIntegrationEventHandler<VocabularyMasteredIntegrationEvent>
{
    private const string HandlerName = nameof(VocabularyMasteredEventHandler);

    public async Task HandleAsync(VocabularyMasteredIntegrationEvent @event, CancellationToken ct = default)
    {
        ...
        var card = await GetOrCreateCardAsync(@event.UserId, @event.MasteredAtUtc, ct);
        card.RecordVocabularyMastered(@event.MasteredAtUtc);
