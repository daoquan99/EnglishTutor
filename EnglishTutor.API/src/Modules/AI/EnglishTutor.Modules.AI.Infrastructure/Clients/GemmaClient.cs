...
public sealed class GemmaClient : IAiClient
{
    public Task<AiResponse> SendAsync(AiRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            AiTaskType.SentenceCorrection => CreateSentenceCorrectionResponse(request.Prompt),
            AiTaskType.AssessmentGrading => CreateAssessmentGradingResponse(request.Prompt),
            _ => request.Prompt,
        };
