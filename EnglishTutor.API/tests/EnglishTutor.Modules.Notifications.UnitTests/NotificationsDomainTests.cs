...
public sealed class NotificationsDomainTests
{
    ...
    {
        var settings = NotificationSetting.Create(Guid.NewGuid(), "Asia/Ho_Chi_Minh", BaseUtcNow);
    ...
    {
        var settings = NotificationSetting.Create(Guid.NewGuid(), "UTC", BaseUtcNow);
    ...
    {
        var schedule = UserNotificationSchedule.CreateDefault(Guid.NewGuid(), NotificationType.StudyReminder, BaseUtcNow);
    ...
    {
        var schedule = UserNotificationSchedule.CreateDefault(Guid.NewGuid(), NotificationType.WeeklyProgressSummary, BaseUtcNow);
    ...
    {
        var (settings, schedules) = NotificationSettingsInitializer.CreateDefaults(Guid.NewGuid(), "UTC", BaseUtcNow);
    ...
    public void Message_MarkAsRead_Sets_Read_Status()
    {
        ...
        message.MarkAsRead(BaseUtcNow.AddMinutes(1));
    ...
    public void Message_MarkAsFailed_Adds_Delivery_Log()
    {
        ...
        message.MarkAsFailed("smtp unavailable", BaseUtcNow.AddMinutes(1));
    ...
    public void Message_MarkAsSent_Sets_Sent_Status_And_Log()
    {
        ...
        message.MarkAsSent(BaseUtcNow.AddMinutes(1));
    ...
    {
        var template = NotificationTemplate.Create(NotificationType.StudyReminder, "EN", "Title", "Body", BaseUtcNow);
    ...
    public async Task PlannedStudySessionMissedHandler_Creates_Notification_And_Marks_Inbox()
    {
        ...
        var handler = new PlannedStudySessionMissedEventHandler(
            ...
            new FakeRealtimeNotificationSender(),
    ...
    private sealed class FakeNotificationRepository : INotificationRepository
    {
        ...
        public Task<IReadOnlyList<NotificationMessage>> GetUnreadForUserAsync(Guid userId, CancellationToken cancellationToken) =>
            Task.FromResult<IReadOnlyList<NotificationMessage>>([.. Messages.Where(m => !m.IsRead)]);
    ...
    private sealed class FakeRealtimeNotificationSender : IRealtimeNotificationSender
    {
        public Task SendNotificationAsync(Guid userId, NotificationPushPayload payload, CancellationToken cancellationToken = default) =>
            Task.CompletedTask;
    }
    ...
    private sealed class FakeScheduleRepository(bool enabled = true) : IUserNotificationScheduleRepository
    {
        ...
        {
            var schedule = UserNotificationSchedule.CreateDefault(userId, type, BaseUtcNow);
            if (enabled)
            {
                schedule.Enable();
            }
    ...
    private sealed class FakeSettingRepository : INotificationSettingRepository
    {
        public Task<NotificationSetting?> GetByUserIdAsync(Guid userId, CancellationToken cancellationToken) =>
            Task.FromResult<NotificationSetting?>(NotificationSetting.Create(userId, "UTC", BaseUtcNow));
    ...
    {
        public DateTime UtcNow => NotificationsDomainTests.BaseUtcNow;
    }
}
