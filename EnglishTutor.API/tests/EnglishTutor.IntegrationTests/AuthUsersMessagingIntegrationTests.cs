    ...
    : IClassFixture<DatabaseEnglishTutorApiFactory>
{
    ...
    public async Task RegisterEndpoint_ShouldPersistAuthUserAndOutboxMessage()
    {
        ...
        (await authDbContext.AuthUsers.AnyAsync(user => user.Email == Email.Create(email))).Should().BeTrue();
        var outboxMessage = await authDbContext.OutboxMessages
            .SingleAsync(message => message.SourceModule == "auth" && message.Payload.Contains(email));
    ...
    public async Task WorkerOutboxProcessing_ShouldProjectRegisteredUserAndRemainIdempotent()
    {
        ...
        var authUser = await authDbContext.AuthUsers.SingleAsync(user => user.Email == Email.Create(email));
        var outboxMessage = await authDbContext.OutboxMessages.SingleAsync(message =>
            message.SourceModule == "auth"
