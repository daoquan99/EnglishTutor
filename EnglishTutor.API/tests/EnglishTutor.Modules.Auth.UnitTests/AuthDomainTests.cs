...
public sealed class AuthDomainTests
{
    ...
    {
        var user = AuthUser.Register(Email.Create("learner@example.com"), " Learner ", BaseUtcNow);
    ...
    public void RefreshToken_Create_Binds_User_Session_And_Hash()
    {
        ...
        var token = RefreshToken.Create(userId, sessionId, "hash", BaseUtcNow.AddDays(7), BaseUtcNow);

        Assert.Equal(userId, token.AuthUserId);
        Assert.Equal(sessionId, token.SessionId);
        Assert.Equal("hash", token.TokenHash);
        Assert.True(token.IsActive(BaseUtcNow));
    }
    ...
    public void RefreshToken_Revoke_Stores_Replacement_Id()
    {
        ...
        var token = RefreshToken.Create(Guid.NewGuid(), Guid.NewGuid(), "hash", BaseUtcNow.AddDays(7), BaseUtcNow);

        token.Revoke(BaseUtcNow, replacementId);
    ...
    {
        var session = AuthSession.Create(Guid.NewGuid(), " device-1 ", "agent", "127.0.0.1", BaseUtcNow);
        var originalLastUsed = session.LastUsedAtUtc;

        session.MarkUsed(BaseUtcNow.AddMinutes(1));
    ...
    {
        var role = AuthRole.Create(" Admin ", "Full access", isSystem: true, BaseUtcNow);
        var permission = AuthPermission.Create(" AI.Providers.Read ", "Read AI providers", BaseUtcNow);
    ...
    public void AuthRolePermission_Binds_Role_And_Permission()
    {
        ...
        var rolePermission = AuthRolePermission.Create(roleId, permissionId, BaseUtcNow);
    ...
    {
        var role = AuthRole.Create(" Content_Admin ", "Manage content", isSystem: false, BaseUtcNow);

        role.Update(" Learning_Admin ", "Manage learning content", isEnabled: false, BaseUtcNow.AddMinutes(1));
    ...
    private sealed class FakeAuthRepository(bool emailExists = false) : IAuthRepository, IAuthUnitOfWork
    {
        ...
        public Task<(IReadOnlyList<AuthUser> Items, int Total)> ListAsync(
            string? search,
        ...
        {
            IReadOnlyList<AuthUser> items = User is null ? [] : [User];
    ...
    private sealed class FakeJwtTokenGenerator : IJwtTokenGenerator
    {
        public GeneratedAccessToken Generate(AuthUser user, IReadOnlyCollection<string> permissionCodes) =>
            new("access-token", BaseUtcNow.AddMinutes(15));
    }

    private sealed class FakeRefreshTokenGenerator : IRefreshTokenGenerator
    {
        public GeneratedRefreshToken Generate() => new("refresh-token", "refresh-token-hash", BaseUtcNow.AddDays(7));
    }
    ...
    {
        public DateTime UtcNow => AuthDomainTests.BaseUtcNow;
    }
}
