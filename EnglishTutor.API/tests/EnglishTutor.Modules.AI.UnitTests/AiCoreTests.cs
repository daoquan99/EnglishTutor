...
public sealed class AiCoreTests
{
    ...
    {
        var template = PromptTemplate.Create("sentence_correction", AiTaskType.SentenceCorrection, "Test template", BaseUtcNow);
        template.AddVersion("System {target_language}", "Sentence {original_text}", activate: true, utcNow: BaseUtcNow);
    ...
    {
        var templates = AiSeedData.CreateDefaultPromptTemplates(BaseUtcNow);
    ...
    {
        public DateTime UtcNow => AiCoreTests.BaseUtcNow;
    }
    ...
    private sealed class FakeAudioStorageService : IAudioStorageService
    {
        ...
        public Task<FileMetadata> UploadAsync(Stream file, string fileName, string contentType, string? folder, CancellationToken ct)
        {
            ...
            Keys.Add(fileKey);
            UploadCount++;
            return Task.FromResult(new FileMetadata(fileKey, fileName, contentType, file.Length, GeneratePresignedUrl(fileKey, TimeSpan.FromHours(1)), BaseUtcNow));
        }
