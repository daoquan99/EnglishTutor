...
{
    [Fact]
    public async Task HandleAsync_Should_Forward_All_Ids_To_Publisher()
    {
        var publisher = new FakeSecurityEventPublisher();
        ...
        var handler = new RefreshTokenReuseAuditHandler(
            publisher, accessor, NullLogger<RefreshTokenReuseAuditHandler>.Instance);
        ...
        var recorded = publisher.Calls[0];
        ...
        recorded.EventType.Should().Be(
            EnglishTutor.Identity.Contracts.Events.IdentitySecurityEventTypes.RefreshTokenReuseDetected);
    }
...
internal sealed class FakeSecurityEventPublisher : IIdentitySecurityEventPublisher
{
    public List<IdentitySecurityEventData> Calls { get; } = new();
